...
export async function systemRoute(app: FastifyInstance): Promise<void> {
  app.get('/api/system/status', async (request) => {
    ...
    return {
      ok: true,
      version: '3.8.0',
      mode: 'admin-dashboard-voice-token-test-no-paid-llm',
      ...
      protectedPages: ['/admin', '/call-leads', '/production-checklist', '/voice-webhook-test', '/unknown-questions', '/website-import', '/weaknesses', '/clients', '/onboarding', '/tts-test'],
      businessSlug,
